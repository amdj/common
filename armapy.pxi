...
# cdef extern from "arma_eigen.h" namespace "math_common":
    # VectorXd eigenMap(double*,us)
...
cdef vd dndtovec(n.ndarray[n.float64_t,ndim=1] ndarr):
    ...
    return varr
# cdef evd dndtoeigen(n.ndarray[n.float64_t,ndim=1] ndarr):
#     cdef us size=ndarr.shape[0]
#     cdef evd varr=eigenMap(&ndarr[0],size)
#     return varr
...
cdef n.ndarray eigentond(VectorXd varr):
    cdef int size=varr.size()
    cdef n.float_t* data = varr.data()
    cdef n.float64_t[:] data_view = <n.float64_t[:size]> data
    cdef n.ndarray[n.float64_t, ndim=1, mode='c'] result
    result = 1.0*n.asarray(data_view, dtype=n.float64, order='C')
#   print('array content in Cython: ' + repr(result))
