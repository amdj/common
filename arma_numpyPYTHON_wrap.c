...
/*-----------------------------------------------
              @(target):= _arma_numpy.so
  ------------------------------------------------*/
#if PY_VERSION_HEX >= 0x03000000
#  define SWIG_init    PyInit__arma_numpy

#else
#  define SWIG_init    init_arma_numpy

#endif
#define SWIG_name    "_arma_numpy"
...
extern "C" {
#endif
SWIGINTERN int Swig_var_vd_set(PyObject *_val) {
  {
    void *argp = 0;
    int res = SWIG_ConvertPtr(_val, &argp, SWIGTYPE_p_class,  0 );
    if (!SWIG_IsOK(res)) {
      SWIG_exception_fail(SWIG_ArgError(res), "in variable '""vd""' of type '""class""'");
    }
    if (!argp) {
      SWIG_exception_fail(SWIG_ValueError, "invalid null reference " "in variable '""vd""' of type '""class""'");
    } else {
      vd = *((class *)(argp));
    }
  }
...
SWIGINTERN PyObject *Swig_var_vd_get(void) {
  PyObject *pyobj = 0;
  
  pyobj = SWIG_NewPointerObj(SWIG_as_voidptr(&vd), SWIGTYPE_p_class,  0 );
  return pyobj;
}
...
# endif
  ...
  SWIG_addvarlink(SWIG_globals(),(char*)"vd",Swig_var_vd_get, Swig_var_vd_set);
#if PY_VERSION_HEX >= 0x03000000
  return m;
#else
  return;
